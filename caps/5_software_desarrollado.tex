...
\addcontentsline{toc}{section}{Desarrollo experimental, comparación de modelos y resultados}
...
- Justificación de los modelos elegidos:
	- MLP (como baseline)
	- Trafficformer (como propuesta avanzada)
- Justificación de los datasets/fuentes usados (explicando brevemente source_id 1, 2, 5 y su relevancia)
- Hipótesis/criterios de éxito: ¿qué esperas demostrar?

\lipsum[1-1]

\subsection{Implementación de los modelos}

- Arquitectura y detalles técnicos de la MLP base
- Arquitectura y detalles técnicos de Trafficformer (incluye aquí la justificación del uso de spatial masks, multi-head attention, etc.)
- Hiperparámetros elegidos (puedes poner tabla resumen de experimentos y combinaciones)

\lipsum[1-1]

\subsection{Descripción de los experimentos}

- Proceso de entrenamiento, validación y test (explica cómo se ha hecho el split, cómo se han preparado los datos, early stopping, métricas usadas, hardware utilizado…)
- Tabla resumen de experimentos realizados
- Justificación de la elección de las métricas: MAE, RMSE, MAPE, R2…

\lipsum[1-1]

\subsection{Resultados obtenidos}

- Presentación de resultados: tablas de métricas, figuras y gráficas de evolución de entrenamiento, validación, etc.
- Comparativa directa entre MLP y Trafficformer, por cada fuente de datos relevante
- Interpretación objetiva de los resultados (sin todavía analizar en profundidad, solo presentar de forma clara y ordenada)

\lipsum[1-1]

\subsection{Discusión y análisis crítico}

- Discusión sobre los resultados: ¿qué modelo ha funcionado mejor y por qué? ¿cómo se comporta Trafficformer frente a la baseline?
- Análisis de las limitaciones encontradas (tanto del dataset como del modelo)
- Comparativa con el estado del arte si procede (¿tus resultados están en línea con la literatura?)
- Reflexión crítica sobre la validez de los resultados y posibles mejoras
...
\begin{comment}
	- Descripción clara de los objetivos del bloque experimental.
	- Justificación de la necesidad de comparar un baseline clásico (MLP) con un modelo SOTA (Trafficformer).
	- Introducción a las fuentes de datos (source_id_1, source_id_2, source_id_5) y su contexto.
\end{comment}
...
%% Puedes añadir aquí una figura tipo mapa con los Meters por source_id si lo tienes visualizado
...
%% Aquí puedes insertar un esquema gráfico de comparación entre MLP vs Trafficformer si lo tienes
...
%% Puedes añadir aquí una tabla resumen de los modelos evaluados por source_id
...
\begin{comment}
	- Detalles sobre el entorno Python, dependencias y hardware usado (incluyendo CUDA, UV, etc.).
	- Cómo se garantiza que los experimentos sean reproducibles (gestión de seeds, versión de código, uso de notebooks, etc.).
	- Referencia a tu repositorio y a la estructura del código.
\end{comment}
...
%% Puedes insertar aquí una captura parcial del pyproject.toml destacando torch/cu128, wandb, pymongo...
\begin{figure}[H]
	\centering
...
%% Aquí puedes añadir una captura del panel de wandb con un ejemplo de experimento (entrenamiento + métricas)
\begin{figure}[H]
	\centering
...
%% Puedes añadir aquí una captura del esquema ONNX exportado, o su validación con onnxruntime
...
%% Puedes insertar aquí un resumen gráfico de la arquitectura desplegada en AWS: EC2 + VPN + MongoDB
...
%% Si lo deseas, puedes incluir el fichero template.yaml como anexocapt_wandb (Anexo C), explicando brevemente su contenido.
...
%% FIN Como se detalla en el \hyperref[anexo:plantilla_aws]{Anexo~C}
...
\begin{comment}
	- Breve explicación del pipeline de datos: de MongoDB al dataset, manejo de features, máscaras espaciales, repository pattern.
	- Explicación de la generación y justificación de máscaras espaciales (destacando el uso real de OSMPathSpatialMaskStrategy).
	- Ejemplo/resumen del código clave (docstring y fragmentos) con referencia al repositorio.
\end{comment}
...
%% Aquí puedes insertar una figura opcional mostrando la máscara resultante para un subconjunto de sensores
...
\begin{comment}
	- Explicación detallada del plan experimental (tablas tipo las de EXPERIMENTS.md).
	- Justificación de los valores elegidos para cada parámetro/hiperparámetro, en línea con el estado del arte y buenas prácticas.
	- Número total de experimentos, cómo se organiza la ejecución (notebooks, Wandb, etc.).
\end{comment}
...
La tabla completa con las 120 combinaciones experimentales puede consultarse en el \hyperref[anexo:combinaciones_exp]{Anexo~E}.
